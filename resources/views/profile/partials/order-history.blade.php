            ...
            @foreach ($orders as $order)
                <div class="accordion-item">
                    <h2 class="accordion-header" id="heading-{{ $order->id }}">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                            ...
                            <div class="d-flex flex-wrap justify-content-between w-100 me-2" style="gap: 10px;">
                                ...
                                <div>
                                    <span class="badge rounded-pill text-bg-{{ $order->status_color }}">
                                        {{ $order->formatted_status }}
                                    </span>

                                    <span class="badge rounded-pill text-bg-{{ $order->payment_status_color }}">
                    ...
                    <div id="collapse-{{ $order->id }}" class="accordion-collapse collapse"
                        aria-labelledby="heading-{{ $order->id }}" data-bs-parent="#orderHistoryAccordion">
                                ...
                                @foreach ($order->items as $item)
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="fw-bold">Rp
                                            {{ number_format($item->quantity * $item->price, 0, ',', '.') }}</div>
                                    </li>
                                @endforeach
