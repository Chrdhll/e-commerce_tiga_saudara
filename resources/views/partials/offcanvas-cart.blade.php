<div class="offcanvas offcanvas-end" tabindex="-1" id="cartDrawer" aria-labelledby="cartDrawerLabel" style="width: 100%; max-width: 450px;">
    ...
    <div class="offcanvas-body p-0 d-flex flex-column">
        @php
            $cartItems = []; // Ganti dengan data keranjang asli, misal: Cart::content()
            $subtotal = 0;   // Ganti dengan logika subtotal, misal: Cart::subtotal()
        @endphp
        ...
        @else
            <div class="flex-grow-1 overflow-auto p-3">
                @foreach($cartItems as $item)
                    <div class="d-flex gap-3 bg-white p-3 rounded shadow-sm mb-3">
                        <img src="{{ $item->model->image }}" alt="{{ $item->name }}" class="rounded" style="width: 80px; height: 80px; object-fit: cover;">
                        <div class="flex-grow-1">
                            ...
                            <form action="{{ route('cart.update', $item->rowId) }}" method="POST" class="d-flex align-items-center">
                                ...
                                <input type="text" class="form-control form-control-sm text-center" value="{{ $item->qty }}" readonly style="width: 40px; height: 30px; margin: 0 5px;">
                        ...
                        <div class="d-flex flex-column justify-content-between align-items-end">
                            ...
                            <p class="text-primary mb-0">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="border-top p-3 bg-light">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span class="fs-4 text-primary fw-bold">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                </div>
                <a href="https://wa.me/6281234567890" target="_blank" class="btn w-100 fs-6 py-2" style="background-color: #25D366; color: white;">
