            ...
            <div class="d-flex flex-column h-100">
                <div class="flex-grow-1 overflow-auto p-3">
                    ...
                    <template x-for="item in $store.cart.items" :key="item.rowId">
                        <div class="d-flex gap-3 bg-white p-3 rounded shadow-sm mb-3">
                            <img :src="item.options.image_url" :alt="item.name" class="rounded" style="width: 80px; height: 80px; object-fit: cover;">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" class="btn btn-outline-secondary btn-sm" style="width: 30px; height: 30px;"
                            ...
                            <div class="d-flex flex-column justify-content-between align-items-end">
                                ...
                                <p class="text-primary mb-0 fw-bold" x-text="`Rp ${(item.price * item.qty).toLocaleString('id-ID')}`"></p>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="border-top p-3 bg-light">
                    ...
                    <form action="{{ route('cart.checkout') }}" method="POST">
                        @csrf
                        <button type="submit" class="btn w-100 fs-6 py-2" style="background-color: #25D366; color: white;">
